...
const createServer = () => {
  ...
  function shortBio(request, response){
    response.status(200).send('A short bio about you to the front-end.');
  }

  app.get('/favoritefoods', favoriteFoods);
  function favoriteFoods(request, response){
    response.status(200).send(['pizza', 'tacos', 'chicken']);
  }
...
const count = (target, input) => {
  let count = 0;
  let numberAmount = input.map(number => {
    number.map(integer => {
      if(integer === target){
        count++;
    }})
...
const totalSum = (input) => {
  let sum = 0;
  let sumArray = input.map(number => number.map(integer => sum += integer));
  return sum;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(fiveArray => fiveArray.reduce((accumulator, int) => {
    int % 5 === 0 && typeof int === 'number' ? accumulator.push(Math.pow(2, int)) : 'blah'
    return accumulator;
  }, []))
};
