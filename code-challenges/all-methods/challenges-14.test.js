...
const createServer = () => {
  ...
  app.get('/', getIt);
  app.delete('/things/1', deleteIt);

  function getIt(request, response){
    response.status(200).send('nice');
  }

  function deleteIt(request, response){
    response.status(405).send('not nice');
  }
...
const sortBy = (property, arr) => {
  let dif = property;
  // let newArray = [];
  let newArray = arr.sort((a, b) => (a.dif > b.dif) ? 1 : -1)
  return newArray;
}
