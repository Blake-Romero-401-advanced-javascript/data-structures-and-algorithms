...
const mapCurrentEvents = () => {
  const eventsArray = currentEvents.news;
  const newArray = eventsArray.map(events => new Event(events));
  // let newArray = new Event.map(currentEvents => {
...
const returnNames = (arr) => {
  let newArray = arr.reduce((accumulator, value, index) => {
