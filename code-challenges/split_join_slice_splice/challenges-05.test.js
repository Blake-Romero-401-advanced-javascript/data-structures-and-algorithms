...
const templateWithJQuery = () => {
  starWarsPeople.forEach(object => {
    const myTemplate = $('#template').html();
    const $newTemplate = $(`<section>${myTemplate}</section>`);
    $newTemplate.find('h2').text(object.name);
    $newTemplate.find('h3').text(object.height);
    $newTemplate.find('p').text(object.eye_color);
...
const listFoods = (recipe) => {
  let result = [];
    recipe.ingredients.forEach(obj => {
      console.log('test from inside loop');
      result.push(obj.slice(8));
    })
